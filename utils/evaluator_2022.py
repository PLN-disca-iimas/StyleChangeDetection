...
def read_solution_files(solutions_folder: str) -> dict:
    ...
    """
    solutions = {}
    for solution_file in glob.glob(os.path.join(solutions_folder, 'prediction-problem-*.json')):
        with open(solution_file, 'r') as fh:
            curr_solution = json.load(fh)
            solutions[re.findall(r'^.*-(problem-[0-9]+).json$',solution_file)[0]] = curr_solution
    return solutions
...
def main():
    ...
    task1_solutions = read_solution_files(os.path.join(args.predictions))
    task1_truth = read_ground_truth_files(os.path.join(args.truth))
    task1_f1 = compute_score_multiple_predictions(task1_truth, task1_solutions, 'changes', labels=[0, 1])

    for k, v in {"task1_f1_score": task1_f1}.items():
